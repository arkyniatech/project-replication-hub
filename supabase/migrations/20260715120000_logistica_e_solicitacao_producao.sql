...
-- 8b. Alinhar logistica_tarefas pré-existente (linhagem 2026 de produção não
--     tinha as colunas de itinerário/execução — o CREATE acima é pulado e os
--     índices/RPCs abaixo quebravam com "column itinerario_id does not exist")
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS itinerario_id UUID REFERENCES logistica_itinerarios(id) ON DELETE SET NULL;
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS check_in_ts TIMESTAMPTZ;
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS check_in_latitude NUMERIC(10, 8);
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS check_in_longitude NUMERIC(11, 8);
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS concluido_ts TIMESTAMPTZ;
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS motivo_falha TEXT;
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS motivo_falha_tipo TEXT;
ALTER TABLE logistica_tarefas ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES auth.users(id);
